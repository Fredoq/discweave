...
public sealed partial class OriginalTrackExternalCandidateEndpointTests
{
    ...
    [Fact(DisplayName = "Low external context remains visible and nonselectable")]
    public async Task Low_external_context_remains_visible_and_nonselectable()
    {
        var candidateId = Guid.Parse(
            "11111111-1111-1111-1111-111111111111");
        var provider = new FakeRecordingLineageProvider
        ...
        ExternalOriginalCandidate candidate =
            Assert.Single(result.Candidates);
        Assert.Equal(OriginalCandidateConfidence.Low, candidate.Ranked.Confidence);
        Assert.False(candidate.Ranked.Selectable);
    }
}
