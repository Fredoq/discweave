...
{
    private static async Task<IResult> AttachLooseFilesToReleaseAsync(
        Guid sessionId,
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ReleaseImportSession? session = await ReleaseImportScanService.AttachLooseFilesToReleaseAsync(
                sessionId,
                request,
                context,
